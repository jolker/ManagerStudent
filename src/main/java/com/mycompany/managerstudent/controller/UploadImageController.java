...
public class UploadImageController extends HttpServlet {
    ...
    private static final String PASSING_URL_IMAGE = "url_image";
    ...
    private void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            IUpload uploadApk = new UploadImageService();
            while (fileItemsIterator.hasNext()) {
                FileItem fileItem = fileItemsIterator.next();
                String folderCreate = uploadApk.createFolder();
                if (StringUtils.isNotBlank(folderCreate)) {
                    String pathResource = String.format("%s/%s", Constants.UPLOAD_RESOURCE, folderCreate);
